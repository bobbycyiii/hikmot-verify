...
def verify_hyperbolicity_hex(M, print_data = False, save_data = False):
...
################ updated 1.9.2014 for v1.0.1 ################

	ind_mat_reduce = []
	for i in range(rowend):
		ind_mat_reduce.append(ind_mat[i][:-1])

	# tmp has row_num rows of ind_mat_reduce
	tmp = []
	for i in range(row_num):
		tmp.append(ind_mat_reduce[i])

	# Index selection by calculating matrix rank
	for i in range(row_num,rowend+1):
		candidate_row = ind_mat_reduce[i]; 
		if RankMat(tmp+[candidate_row]) > row_num:
			tmp.append(candidate_row)
			row_ind.append(i)
			row_num += 1
		if row_num == length_c:
			break
...
#############################################################
	...
	for n in range(len(x0)):
		if n % 2 == 0:
			# hikmot_data += "%+07.16f" % x0[n]
			hikmot_data += '('
			hikmot_data += x0[n].hex()
			hikmot_data += ')+('
		else:
	...
	if x[-1] == 1:
		...
		for i in range(length_c):
			...
			hikmot_data += x[n+1].hex()
			hikmot_data += ']+['
			hikmot_data += x[n+2].hex()
			hikmot_data += ', '
			hikmot_data += x[n+3].hex()
			hikmot_data += ']i\n'
			n += 4
	...
	if arg_verified and (x[-1] == 1):
		hikmot_data += 'verification succeeds'
		if save_data:
...
################
	...
	if x[-1] == 1:
		...
		for i in range(length_c):
			...
			hikmot_data += ', '
			# hikmot_data += x[n+1].hex()
			hikmot_data += "%+07.16f" % x[n+1]
			hikmot_data += ']+['
			# hikmot_data += x[n+2].hex()
			hikmot_data += "%+07.16f" % x[n+2]
			hikmot_data += ', '
			# hikmot_data += x[n+3].hex()
			hikmot_data += "%+07.16f" % x[n+3]
			hikmot_data += ']i\n'
			n += 4
