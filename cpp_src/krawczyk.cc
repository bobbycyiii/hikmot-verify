#define PY_SSIZE_T_CLEAN /* per the CPython documentation for Python 3.8 */
#include <Python.h>
#include <iostream>
...
PyObject* krawczyk(PyObject* self, PyObject* args)
{
	...
	for (i=0; i<n; i++) {
		for (j=0; j<n; j++) {
			value = PyList_GetItem(list, count++);
			f.b(i,j) = PyLong_AsLong(value);
		}
	}

	for (i=0; i<n; i++) {
		value = PyList_GetItem(list, count++);
		f.c(i) = PyLong_AsLong(value);
	}
	...
	if (r == true) value = PyLong_FromLong(1);
	else value = PyLong_FromLong(0);
	PyList_SetItem(result, count++, value);
...
static struct PyModuleDef krawczykmodule = {
    PyModuleDef_HEAD_INIT,
    "krawczyk",
    NULL,
    -1,
/* 
   The above -1 says the module keeps state in global variables. 
   I don't know if that is correct. 
   --bobbycyiii 2020-07-24 
*/
    krawczykmethods
};

PyMODINIT_FUNC
PyInit_krawczyk(void)
{
    return PyModule_Create(&krawczykmodule);
}
